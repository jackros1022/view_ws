...
public:
    ...
    void cloudCB(const sensor_msgs::PointCloud2 &input)
    {
        ...
        coef_pub.publish(ros_coefficients);     //发布模型系数

        // Publish the Point Indices
        pcl_msgs::PointIndices ros_inliers;
        pcl_conversions::fromPCL(*inliers, ros_inliers);
        ind_pub.publish(ros_inliers);           //内部点
        ...
        extract.filter(cloud_segmented);        //分割

        //Publish the new cloud
        sensor_msgs::PointCloud2 output;
        pcl::toROSMsg(cloud_segmented, output);
        pcl_pub.publish(output);                //发布分割的效果
    }

protected:
    ros::NodeHandle nh;
    ros::Subscriber pcl_sub;
    ros::Publisher pcl_pub, ind_pub, coef_pub;    //发布那么多干什么？
};
