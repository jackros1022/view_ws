...
public:
    ...
    : viewer("Cloud Viewer")
    {
        output_sub = nh.subscribe("/kinect2/sd/points", 1, &cloudHandler::outputCB, this);
        downsampled_sub = nh.subscribe("/kinect2/qhd/points", 1, &cloudHandler::downsampledCB, this);
        viewer_timer = nh.createTimer(ros::Duration(0.01), &cloudHandler::timerCB, this);      //设置定时器 100ms毫秒执行一次去执行回调函数
