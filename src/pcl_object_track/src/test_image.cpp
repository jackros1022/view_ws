...
class HSV_tracker
{
 ...
 public:
  ...
  {
    HSV=x;
    sub_2=nh.subscribe<sensor_msgs::Image>("/kinect2/qhd/image_color", 1, &HSV_tracker::image_callback,this);
  }
 private:
...
void HSV_tracker::image_callback(const sensor_msgs::ImageConstPtr& img_ptr)  //函数的实现，同时也是回调函数,不放在类里面
{
  cv::Mat cv_image;
	cv_bridge::CvImagePtr cv_ptr;
  cv_ptr = cv_bridge::toCvCopy(img_ptr);    //变身
...
int main(int argc, char** argv)
{
 ...
 HSV_tracker tracker(HSV);    //传入参数怎么理解？
